...
import { query } from '@/app/lib/db';
import crypto from 'crypto';
import { approveDepositFromWebhook } from '@/app/lib/actions';
...
export async function POST(req: NextRequest) {
  ...
  const amount = form.get('AMOUNT')?.toString();
  const status = form.get('status')?.toString();
  const intid = form.get('intid')?.toString();
  const sign = form.get('SIGN')?.toString();
  const orderId = form.get('MERCHANT_ORDER_ID')?.toString();

  const merchantId = process.env.PAYOU_MERCHANT_ID;
  const secretKey = process.env.PAYOU_SECRET_KEY;

  if (!merchantId || !secretKey || !amount || !status || !intid || !sign || !orderId) {
    return new Response(`${orderId || 'unknown'}|error`, { status: 400 });
  }
  ...
  try {
    ...
    const transaction = rows[0];
    if (!transaction) return new Response(`${orderId}|error`, { status: 404 });
    if (transaction.status !== 'in_process') return new Response(`${orderId}|success`);

    if (parseFloat(amount) !== parseFloat(transaction.amount.toString())) {
      return new Response(`${orderId}|error`, { status: 409 });
    }

    await approveDepositFromWebhook(parseInt(orderId, 10));

    return new Response(`${orderId}|success`);
  } catch (error) {
    console.error('Webhook approve error:', error);
    return new Response(`${orderId}|error`, { status: 500 });
  }
}
